...
% \times\thinspace&\exp\left(-\frac{g\tau^*}{2}[x(t-\tau^*) + x(t)] - 
% M_B - \frac{\mu_M\tau^*}{2}\left(e^{-\rho x(t-\tau^*)} + 
% e^{-\rho x(t)}\right)\right)

% If t < tau*,
% u(t,\tau^*) =  u_0(\tau^*-t)\exp\left(-\frac{g\tau^*}{2}[x(\tau^*-t) + x(t)]
% - M_B 
...
uu2 = -0.5*g*tau*(x_delay + x) - par(15) - 0.5*Mm*tau*(exp(-rho*x_delay)+exp(-rho*x));
uu = uu1*exp(uu2);

dy(1) = (par(1) - par(2)*x + par(6)*y1 - par(4)*y2)*x;

dy(2) = par(3)*x*y2 + (1-exp(-z*x))*par(7)*y2 - par(5)*x*y1 - ...
